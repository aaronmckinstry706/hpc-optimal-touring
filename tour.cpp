...
std::vector<struct site> get_nearby_sites_to_cluster(struct cluster c, std::vector<struct site> sites) {
	...
	// TODO: sort site_distances based on distance (first part of pair), return closest CLOSEST
...
int time_finished_visiting(struct site site,
                           ...
                           int curTime) {
    // std::cout << "going to site " << site
              // << ",\n\thours " << openHours
              // << ",\n\tfrom position " << curPos
              // << ",\n\tat time " << curTime << ":" << std::endl;
    int travelTime;
    if (curTime == 0)
    ...
    // std::cout << "\t\tfinished time is " << finishedTime << std::endl;
...
std::pair<double,std::vector<int> > optimalPath(
    ...
    // std::cout << curPos << ' ' << curTime << std::endl;
    ...
    for (int i = 0; i < sites.size(); ++i) {
        ...
        if (nextTime != -1) {
            ...
            std::pair<double,std::vector<int> > value_remainingPath =
                optimalPath(sites, times, used, nextTime, nextPosition);
            used[i] = false;
...
std::pair<double,std::vector<int> > get_value_around_cluster_for_day(
    ...
    // std::cout << "times to search: " << std::endl;
    ...
    for (auto const& s : nearby) {
        // std::cout << "search for time corresponding to " << s << ", day " << day+1 << "..." << std::endl;
        auto it = std::find_if(times.begin(),times.end(),times_has_day_id(day+1,s.n));
        if (it != times.end()) {
            nearbyTimes.push_back(*it);
            // std::cout << "found time " << *it << std::endl;
        }
    }
    // std::cout << "nearby:" << std::endl << nearby << std::endl
              // << "nearbyTimes:" << std::endl << nearbyTimes << std::endl;
...
int main(int argc, char *argv[]) {
    ...
    // std::cout << "sites: " << std::endl << sites << std::endl;
    // std::cout << "site times: " << std::endl << site_times << std::endl;
    ...
    // std::cout << "clusters: " << std::endl << clusters << std::endl;
    
	std::vector<int> best_tour;
	double best_value = 0;

	// TODO: add in timer so it finishes within 2min
	do {
        ...
        if (clusters.size() == 1)
            break;
              ...
              clusters.begin(),clusters.end(), lexical_cluster_comparison()));
